...
from april.anomalydetection.testaxiombuilder import TestAxiomBuilder
...
test_ltn_class_row_values = [10, 25] 
# + list(range(50, 351, 50))
# test_ltn_class_row_values = [10, 25]
test_ltn_row_classes = []
test_leaky_class_row_values = [10, 25] 
# + list(range(50, 351, 50))
# test_leaky_class_row_values = [10, 25]
test_leaky_row_classes = []

class TestDAE(NNAnomalyDetector):
    """Implements a denoising autoencoder based anomaly detection algorithm."""

    abbreviation = f'testdae'
    name = f'TestDAE'
    ...
    def __init__(self, model=None):
        ...
        super(TestDAE, self).__init__(model=model)
    ...
    def model_fn(cls, dataset, **kwargs):
        ...
        # Load ltn rows
        with open('test_ltn_rows.pkl', 'rb') as f:
...
class TestLTN(NNAnomalyDetector):
    """Implements a denoising autoencoder based anomaly detection algorithm."""

    abbreviation = f'Testltn'
    name = f'TestLTN'
    ...
    def __init__(self, model=None):
        super(TestLTN, self).__init__(model=model)
    ...
    def _build_ltn_axioms(self):
        ...
        self.axiombuilder = TestAxiomBuilder(self.activity_predicates, self.activity_constants)
...
class TestLTNFROZEN(TestLTN):
    """Implements a denoising autoencoder based anomaly detection algorithm."""

    abbreviation = f'testltnfrozen'
    name = f'TestLTNFROZEN'

    def __init__(self, model=None):
        super(TestLTNFROZEN, self).__init__(model=model)
...
def create_ltn_classes(base_class, row_values, target_module):
    for rows in row_values:
        ...
        test_ltn_row_classes.append(new_class)
...
def create_leaky_classes(base_class, leaky_values, target_module):
    for leaky in leaky_values:
        ...
        setattr(target_module, class_name, new_class)  # Register to actual module
        test_leaky_row_classes.append(new_class)
# Create and register the classes
create_ltn_classes(TestLTNFROZEN, test_ltn_class_row_values, sys.modules[__name__])
create_leaky_classes(TestDAE, test_leaky_class_row_values, sys.modules[__name__])