...
ltn_rows_file = 'large_ltn_rows.pkl'
large_ltn_class_row_values = [10, 25] + list(range(50, 351, 50))
# large_ltn_class_row_values = [10, 25]
large_ltn_row_classes = []
large_leaky_class_row_values = [10, 25] + list(range(50, 351, 50))
# large_leaky_class_row_values = [10, 25]
large_leaky_row_classes = []

class LargeDAE(NNAnomalyDetector):
    """Implements a denoising autoencoder based anomaly detection algorithm."""

    abbreviation = f'largedae'
    name = f'LargeDAE'
    ...
    def __init__(self, model=None):
        ...
        super(LargeDAE, self).__init__(model=model)
...
class LargeLTN(NNAnomalyDetector):
    """Implements a denoising autoencoder based anomaly detection algorithm."""

    abbreviation = f'Largeltn'
    name = f'LargeLTN'
    ...
    def __init__(self, model=None):
        super(LargeLTN, self).__init__(model=model)
...
class LargeLTNFROZEN(LargeLTN):
    """Implements a denoising autoencoder based anomaly detection algorithm."""

    abbreviation = f'largeltnfrozen'
    name = f'LargeLTNFROZEN'

    def __init__(self, model=None):
        super(LargeLTNFROZEN, self).__init__(model=model)
...
def create_ltn_classes(base_class, row_values, target_module):
    for rows in row_values:
        ...
        large_ltn_row_classes.append(new_class)
...
def create_leaky_classes(base_class, leaky_values, target_module):
    for leaky in leaky_values:
        ...
        setattr(target_module, class_name, new_class)  # Register to actual module
        large_leaky_row_classes.append(new_class)
# Create and register the classes
create_ltn_classes(LargeLTNFROZEN, large_ltn_class_row_values, sys.modules[__name__])
create_leaky_classes(LargeDAE, large_leaky_class_row_values, sys.modules[__name__])