...
ltn_rows_file = 'wide_ltn_rows.pkl'
wide_ltn_class_row_values = [10, 25] + list(range(50, 301, 50))
# wide_ltn_class_row_values = [10, 25]
wide_ltn_row_classes = []
wide_leaky_class_row_values = [10, 25] + list(range(50, 301, 50))
# wide_leaky_class_row_values = [10, 25]
wide_leaky_row_classes = []

class WideDAE(NNAnomalyDetector):
    """Implements a denoising autoencoder based anomaly detection algorithm."""

    abbreviation = f'widedae'
    name = f'WideDAE'
    ...
    def __init__(self, model=None):
        ...
        super(WideDAE, self).__init__(model=model)
...
class WideLTN(NNAnomalyDetector):
    """Implements a denoising autoencoder based anomaly detection algorithm."""

    abbreviation = f'Wideltn'
    name = f'WideLTN'
    ...
    def __init__(self, model=None):
        super(WideLTN, self).__init__(model=model)
...
class WideLTNFROZEN(WideLTN):
    """Implements a denoising autoencoder based anomaly detection algorithm."""

    abbreviation = f'wideltnfrozen'
    name = f'WideLTNFROZEN'

    def __init__(self, model=None):
        super(WideLTNFROZEN, self).__init__(model=model)
...
def create_ltn_classes(base_class, row_values, target_module):
    for rows in row_values:
        ...
        wide_ltn_row_classes.append(new_class)
...
def create_leaky_classes(base_class, leaky_values, target_module):
    for leaky in leaky_values:
        ...
        setattr(target_module, class_name, new_class)  # Register to actual module
        wide_leaky_row_classes.append(new_class)
# Create and register the classes
create_ltn_classes(WideLTNFROZEN, wide_ltn_class_row_values, sys.modules[__name__])
create_leaky_classes(WideDAE, wide_leaky_class_row_values, sys.modules[__name__])