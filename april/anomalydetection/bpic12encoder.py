...
bpic12_ltn_class_row_values = [10, 25] + list(range(50, 401, 50))
bpic12_ltn_row_classes = []
bpic12_leaky_class_row_values = [10, 25] + list(range(50, 401, 50))
bpic12_leaky_row_classes = []

class Bpic12DAE(NNAnomalyDetector):
    """Implements a denoising autoencoder based anomaly detection algorithm."""

    abbreviation = 'bpic12dae'
    name = 'Bpic12DAE'
    ...
    def __init__(self, model=None):
        ...
        super(Bpic12DAE, self).__init__(model=model)
...
class Bpic12LTN(NNAnomalyDetector):
    """Implements a denoising autoencoder based anomaly detection algorithm."""

    abbreviation = 'Bpic12ltn'
    name = 'Bpic12LTN'
    ltn_rows = 400 # rows to use for LTN training
    ...
    def __init__(self, model=None):
        super(Bpic12LTN, self).__init__(model=model)
    ...
    def _build_ltn_constants(self, dataset):
        ...
        return self.activity_constants
...
class Bpic12LTNFROZEN(Bpic12LTN):
    """Implements a denoising autoencoder based anomaly detection algorithm."""

    abbreviation = 'bpic12ltnfrozen'
    name = 'Bpic12LTNFROZEN'

    def __init__(self, model=None):
        super(Bpic12LTNFROZEN, self).__init__(model=model)
...
def create_ltn_classes(base_class, row_values, target_module):
    for rows in row_values:
        ...
        bpic12_ltn_row_classes.append(new_class)
...
def create_leaky_classes(base_class, leaky_values, target_module):
    for leaky in leaky_values:
        ...
        setattr(target_module, class_name, new_class)  # Register to actual module
        bpic12_leaky_row_classes.append(new_class)

# Create and register the classes
create_ltn_classes(Bpic12LTNFROZEN, bpic12_ltn_class_row_values, sys.modules[__name__])
create_leaky_classes(Bpic12DAE, bpic12_leaky_class_row_values, sys.modules[__name__])
