...
ltn_rows_file = 'small_ltn_rows.pkl'
small_ltn_class_row_values = [10, 25] + list(range(50, 351, 50))
# small_ltn_class_row_values = [10, 25]
small_ltn_row_classes = []
small_leaky_class_row_values = [10, 25] + list(range(50, 351, 50))
# small_leaky_class_row_values = [10, 25]
small_leaky_row_classes = []

class SmallDAE(NNAnomalyDetector):
    """Implements a denoising autoencoder based anomaly detection algorithm."""

    abbreviation = f'smalldae'
    name = f'SmallDAE'
    ...
    def __init__(self, model=None):
        ...
        super(SmallDAE, self).__init__(model=model)
...
class SmallLTN(NNAnomalyDetector):
    """Implements a denoising autoencoder based anomaly detection algorithm."""

    abbreviation = f'Smallltn'
    name = f'SmallLTN'
    ...
    def __init__(self, model=None):
        super(SmallLTN, self).__init__(model=model)
...
class SmallLTNFROZEN(SmallLTN):
    """Implements a denoising autoencoder based anomaly detection algorithm."""

    abbreviation = f'smallltnfrozen'
    name = f'SmallLTNFROZEN'

    def __init__(self, model=None):
        super(SmallLTNFROZEN, self).__init__(model=model)
...
def create_ltn_classes(base_class, row_values, target_module):
    for rows in row_values:
        ...
        small_ltn_row_classes.append(new_class)
...
def create_leaky_classes(base_class, leaky_values, target_module):
    for leaky in leaky_values:
        ...
        setattr(target_module, class_name, new_class)  # Register to actual module
        small_leaky_row_classes.append(new_class)
# Create and register the classes
create_ltn_classes(SmallLTNFROZEN, small_ltn_class_row_values, sys.modules[__name__])
create_leaky_classes(SmallDAE, small_leaky_class_row_values, sys.modules[__name__])