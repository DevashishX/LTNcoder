...
class P2PDAE(NNAnomalyDetector):
    ...
    def _build_main_model(self):
        ...
        self.flat_input = self.input_layer
...
class P2PDAELTN(NNAnomalyDetector):
    ...
    def __init__(self, dataset:april.Dataset = dataset, model=None, hidden_layers=None, hidden_size_factor=None, noise=None):
        """Initialize DAE model.
        ...
        self.noise = noise if noise is not None else True # for denoising autoencoder
        self.input_shape = self.dataset.flat_onehot_features_2d.shape[1]
        self.dataset_attribute_dims = self.dataset.attribute_dims.astype(np.int32)
        # self.dataset_attribute_dims = self.dataset.attribute_dims
    ...
    def _fit_main_model(self, epochs=50, batch_size=100, validation_split=0.1, **kwargs):
        """
    ...
    def _build_individual_models(self):
        ...
        while offset < total_output_dim:
            for dim, name in zip(self.dataset_attribute_dims, self.dataset.attribute_keys):
    ...
    def _build_ltn_axioms(self):
        ...
        def axioms(features, labels=None, training=False):
            ...
            axioms = [
                Forall(traces, self.individual_predicates[0]([traces, self.activity_constants["▶"]], training=training))           
            ]
    ...
    def fit(
        ...
        self._fit_main_model(epochs=20, batch_size=batch_size, validation_split=0.1)
        # return
